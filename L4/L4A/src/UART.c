...
{
	RCC->APB2ENR |= RCC_APB2ENR_USART1EN; // ENABLE THE USART1 CLOCK IN PERIPHERAL CLOCK REGISTER
	RCC->CCIPR &= ~RCC_CCIPR_USART1SEL;
	RCC->CCIPR |= RCC_CCIPR_USART1SEL_0; // SYSTEM CLOCK SET TO USART1 CLOCK SOURCE
}
...
void UART2_Init(void)
{
	...
	RCC->CCIPR |= RCC_CCIPR_USART2SEL_0; // SYSTEM CLOCK SET TO USART2 CLOCK SOURCE
}

void UART1_GPIO_Init(void)
{
	// [TODO]
	RCC->AHB2ENR |= RCC_AHB2ENR_GPIOBEN; // ENABLE CLOCK FOR GPIO PIN B
	GPIOB->OSPEEDR &= ~(GPIO_OSPEEDER_OSPEEDR6 | GPIO_OSPEEDER_OSPEEDR7);
	GPIOB->OSPEEDR |= GPIO_OSPEEDER_OSPEEDR6;
	GPIOB->OSPEEDR |= GPIO_OSPEEDER_OSPEEDR7; // SETS GPIO PORT B PINS 6 AND 7 TO HIGH SPEED

	GPIOB->OTYPER &= ~GPIO_OTYPER_OT6;
	GPIOB->OTYPER &= ~GPIO_OTYPER_OT7; // SET GPIO PORT B PINS 6 AND 7 TO PUSH-PULL OUPUT TYPE

	GPIOB->PUPDR &= ~GPIO_PUPDR_PUPD6;
	GPIOB->PUPDR &= ~GPIO_PUPDR_PUPD7;
	GPIOB->PUPDR |= GPIO_PUPDR_PUPD6_0;
	GPIOB->PUPDR |= GPIO_PUPDR_PUPD7_0; // SET GPIO PORT B PINS 6 AND 7 TO USE PULL-UP RESISTORS FOR IO

	// MODER//
	GPIOB->MODER &= ~GPIO_MODER_MODE6;	// CLEAR B6
	GPIOB->MODER |= GPIO_MODER_MODE6_1; // SET B6

	GPIOB->MODER &= ~GPIO_MODER_MODE7;	// CLEAR B7
	GPIOB->MODER |= GPIO_MODER_MODE7_1; // SET B7

	GPIOB->AFR[0] &= ~GPIO_AFRL_AFSEL6;												 // CLEAR
	GPIOB->AFR[0] |= (GPIO_AFRL_AFSEL6_2 | GPIO_AFRL_AFSEL6_1 | GPIO_AFRL_AFSEL6_0); // SET TO ALTERNATIVE FUNCTION

	GPIOB->AFR[0] &= ~GPIO_AFRL_AFSEL7;												 // CLEAR
	GPIOB->AFR[0] |= (GPIO_AFRL_AFSEL7_2 | GPIO_AFRL_AFSEL7_1 | GPIO_AFRL_AFSEL7_0); // SET TO ALTERNATIVE FUNCTION
}
...
void USART_Write(USART_TypeDef *USARTx, uint8_t *buffer, uint32_t nBytes)
{
	...
	for (i = 0; i < nBytes; i++)
	{
		...
		USART_Delay(40000); // changing from 300 to 4000 for USART1
	}
