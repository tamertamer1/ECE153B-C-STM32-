...
void I2C_Initialization(void){
	...
	RCC->CCIPR |= RCC_CCIPR_I2C1SEL_0;                //set system clk as source
	RCC->APB1RSTR1 |= RCC_APB1RSTR1_I2C1RST;            //reset
	RCC->APB1RSTR1 &= ~(RCC_APB1RSTR1_I2C1RST);         //clear
	I2C1->CR1 &= ~(I2C_CR1_PE);           //disable
	I2C1->CR1 &= ~I2C_CR1_ANFOFF;          //enable analog noise
	I2C1->CR1 &= ~(I2C_CR1_DNF);                  //disable digital noise
	I2C1->CR1 |= I2C_CR1_ERRIE;           //enable error interrupt
	I2C1->CR1 &= ~I2C_CR1_NOSTRETCH;      //enable clock stretch
	I2C1->CR2 &= ~I2C_CR2_ADD10; //set master to 7bit
	...
	I2C1->TIMINGR |= 7UL<<28; //PRESC     [TODO]
	I2C1->TIMINGR |= 1UL<<20; //SCLDEL    [TODO]
	I2C1->TIMINGR |= 1UL<<16; //SDADEL    [TODO]
	I2C1->TIMINGR |= 5UL<<8; //SCLH      [TODO]
	I2C1->TIMINGR |= 5UL; //SCLL    [TODO]
	I2C1->OAR1 &= ~I2C_OAR1_OA1EN; //disable own address
	I2C1->OAR1 &= ~I2C_OAR1_OA1MODE; //set to 7bit
	I2C1->OAR1 &= ~I2C_OAR1_OA1;
	I2C1->OAR1 |= OwnAddr; //write address
...
int8_t I2C_ReceiveData(I2C_TypeDef * I2Cx, uint8_t DeviceAddress, uint8_t *pData, uint8_t Size) {
	...
	I2C_Start(I2Cx, DeviceAddress, Size, READ_FROM_SECONDARY); // 0 = sending data to the slave, 1 = receiving data from the slave
